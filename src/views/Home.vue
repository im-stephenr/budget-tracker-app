...
const changeMonth = (type) => {
  ...
  if (type == "next" && current_month_index !== -1) {
    ...
    loadBudget(current_month_index, form_filter.value.year);
    loadTransactions(current_month_index, form_filter.value.year);
      ...
      >
        ...
        <div class="w-full bg-gray-200 rounded-full mt-4">
          ...
          >
            {{
              budget_data?.amount
                ? ((purpose.total_expense / budget_data.amount) * 100).toFixed(
                    2
                  )
                : "0.00"
            }}%
